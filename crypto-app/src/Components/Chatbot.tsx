...
export function Chatbot() {
  ...
  return (
    <div className="fixed bottom-8 right-8 z-[1000] flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 w-96 h-[500px] bg-[--bg-primary] border border-[--border] rounded-[2rem] shadow-2xl overflow-hidden flex flex-col animate-in slide-in-from-bottom-8 duration-300 backdrop-blur-xl">
          {/* Header */}
          <div className="p-6 border-b border-[--border] bg-[--surface] flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-[--brand]/10 border border-[--brand]/20 rounded-xl flex items-center justify-center">
                <VaultaLogo size={20} className="text-[--brand]" />
              </div>
              <div>
                <h3 className="text-[--text-primary] font-black text-sm tracking-tight">Vaulta Assistant</h3>
                <div className="flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" />
                  <span className="text-green-400 text-[8px] font-black uppercase tracking-widest">Active</span>
                </div>
              </div>
            </div>
          ...
          <div className="p-6 border-t border-[--border]">
            <div className="relative flex items-center gap-2">
              <input 
                ...
                placeholder="Message..."
                className="flex-1 bg-[--surface] border border-[--border] rounded-xl px-4 py-3 text-xs text-[--text-primary] outline-none focus:border-[--brand] transition-all"
              />
              <button 
                onClick={handleSend}
                disabled={!inputValue.trim()}
                className="p-3 bg-[--brand] hover:bg-[--brand-dim] text-white rounded-xl transition-all disabled:opacity-30"
              >
                <Send size={16} />
              </button>
            </div>
      ...
      <button 
        ...
        className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-500 hover:scale-110 active:scale-95 shadow-xl ${
          isOpen ? 'bg-[--surface] text-[--text-primary] rotate-90 border border-[--border]' : 'bg-[--brand] text-white'
        }`}
      >
        {isOpen ? <X size={28} /> : <VaultaLogo size={32} className="animate-pulse" />}
        {!isOpen && (
          <span className="absolute -top-1 -right-1 w-5 h-5 bg-green-500 border-4 border-[--bg-primary] rounded-full" />
        )}
      </button>
    </div>
  );
